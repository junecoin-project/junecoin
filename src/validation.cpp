...
#if defined(NDEBUG)
# error "JuneCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "JuneCoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s JUC additional fees, %d delta bytes\n",
...
{
    if (nHeight <= 262800 ){
        return 9 * INITIAL_BLOCK_REWARD * COIN;
    }
...
void ThreadScriptCheck() {
    RenameThread("junecoin-scriptch");
